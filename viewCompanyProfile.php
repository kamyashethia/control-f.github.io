...
<body>
	...
	<div class = "container-fluid" id = "top-background">
      ...
      <?php #query to get user information#
   				$query = "SELECT cName FROM company WHERE compID = 17 ";
	...
	<div class="container-fluid">
		...
		<div class="row">
          ...
          <?php #query to get user information#
       				$query = "SELECT cDescription FROM company WHERE compID = 17 ";
       				if ( ! ( $result = mysqli_query($conn, $query)) ) {
       					echo("Error: %s\n"+ mysqli_error($conn));
       					exit(1);
       				}
          ...
          <?php #query to get user information#
              $query = "SELECT Founder, Location, Focus FROM company WHERE compID = 17";
              if ( ! ( $result = mysqli_query($conn, $query)) ) {
                echo("Error: %s\n"+ mysqli_error($conn));
                exit(1);
              }
              while($row = mysqli_fetch_assoc($result)) {
              echo("Location: &nbsp; <span contentEditable='false' class='facts-text'> " . $row['Location'] . "</span><br>");
              echo("Founder: &nbsp; <span contentEditable='false' class='facts-text' >" . $row['Founder'] . "</span><br>");
              echo("Focus: &nbsp; <span contentEditable='false' class='facts-text' >" . $row['Focus'] . "</span><br>");
            }
            ?>
          ...
          <?php #query to get user information#
              $query = "SELECT cEmail, cPhoneNumber FROM company WHERE compID = 17;";
              if ( ! ( $result = mysqli_query($conn, $query)) ) {
                echo("Error: %s\n"+ mysqli_error($conn));
                exit(1);
              }
              while($row = mysqli_fetch_assoc($result)) {
              echo("Email: &nbsp; <span contentEditable='false' class='facts-text'> " . $row['cEmail'] . "</span><br>");
              echo("Phone: &nbsp; <span contentEditable='false' class='facts-text' >" . $row['cPhoneNumber'] . "</span><br>");
              }
            ?>
        ...
        <center>
          ...
          <?php #query to get user information#
              $query = "SELECT title,post_date,aDescription FROM advert WHERE compID = 17;";
              if ( ! ( $result = mysqli_query($conn, $query)) ) {
                echo("Error: %s\n"+ mysqli_error($conn));
                exit(1);
              }
